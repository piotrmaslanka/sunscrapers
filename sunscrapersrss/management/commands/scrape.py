...
class Command(BaseCommand):
    ...
    def scrape_for_currency(self, currency_code):
        req = requests.get(
            'https://www.ecb.europa.eu/rss/fxref-%s.html' % (currency_code,))
        try:
        ...
        for obj in obj.rdf_RDF.item:
            ...
            try:
                currency_rate = CurrencyRate.objects.get(
                    currency_from=base_currency, currency_to=target_currency,
                    currency_date=date)
            except CurrencyRate.DoesNotExist:
                currency_rate = CurrencyRate(currency_from=base_currency,
